...
void MetadataOrchestrator::Store(Data &original_request,
  ...
  if (exists) {
    ...
    for (auto distribution : distributions) {
      distribution.destination_data_.buffer_ = std::string().data();
      primary_metadata.links_.insert({distribution.destination_data_.position_,
                                      distribution.destination_data_});
    }
  }
  if (is_metadata_updated) {
    ...
    basket::Singleton<IOFactory>::GetInstance()
        ->GetIOClient(original_metadata.storage_index_)
        ->Write(original_metadata, original_metadata);
  }
  if (!exists) {
    ...
    for (auto solution : SYMBIOS_CONF->STORAGE_SOLUTIONS) {
      ...
      if (!is_primary) {
        ...
        basket::Singleton<IOFactory>::GetInstance()
            ->GetIOClient(link_meta.storage_index_)
            ->Write(link_meta, link_meta);
      }
    }
  }
...
MetadataOrchestrator::Locate(Data &request, Metadata &primary_metadata) {
  ...
  auto tracer = common::debug::AutoTrace(
      std::string("MetadataOrchestrator::Locate"), request);
  ...
  basket::Singleton<IOFactory>::GetInstance()
      ->GetIOClient(original_metadata.storage_index_)
      ->Read(original_metadata, original_metadata);
  clmdep_msgpack::object_handle oh = clmdep_msgpack::unpack(
      (char *)original_metadata.buffer_, original_metadata.data_size_);
  ...
  for (auto link : primary_metadata.links_) {
    if (link.first >= request.position_ &&
        link.first <= request.position_ + request.data_size_) {
      auto dest_data = link.second;
      if (request.position_ > link.first)
        dest_data.position_ = request.position_;
      if (request.position_ + request.data_size_ <
          link.first + link.second.data_size_)
