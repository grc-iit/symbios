...
static char doc[] =
"Synthetic Symbios metadata evaluation (Redis)";
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
  ...
  switch (key) {
    ...
    case 'r':
      arguments->redis_hosts = arg; break;
                   ...
                   int count)
{
  ...
  if (rank == 0) {
    printf("Throughput of %s (Redis): %lf op/s\n",
...
void write_test(struct arguments *conf, char *key_base)
{
  ...
  for (i = 0; i < conf->count; i++) {
    ...
    create_symbios_metadata_redis(&metadata);
  }
}
...
void read_test(struct arguments *conf, char *key_base)
{
  ...
  for (i = 0; i < conf->count; i++) {
    sprintf(key, "%s_%d_%d_%d", key_base, rank, rand(), i);
    // printf("%s\n", key);
    query_symbios_metadata_by_id_redis(key);
  }
}
...
int main(int argc, char *argv[])
{
  ...
  ret = connectToRedisServerWithHostfile(conf.redis_hosts);
